...
public class FileAse256 {

	private static final String algorithm = "AES";
	private static final String transformation = algorithm + "/CBC/PKCS5Padding";
	private static final String ivStr = algorithm + "1223453671890";

	private Key key;

	public FileAse256(Key key) {
		this.key = key;
	}
	...
	public void decrypt(S3ObjectInputStream is, OutputStream os) throws Exception{
	...
	private void crypt(int mode, File source, File dest) throws Exception {
		...
		IvParameterSpec ivParmeterSpce = new IvParameterSpec(iv);
		/* CBC 변경으로 추가되는 부분 */
		
		Cipher cipher = Cipher.getInstance(transformation);
//		cipher.init(mode, key);
		/* CBC 변경으로 변경되는 부분 */
		cipher.init(mode, key, ivParmeterSpce);
		/* CBC 변경으로 변경되는 부분 */
